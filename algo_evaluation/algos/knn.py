...
class KNN:
    def __init__(self, data, n_neighbours=3):
        ...
        LOGGER.info('Created KNN classifier with %s neighbours = %s', n_neighbours)
    ...
    def train(self):
        ...
        """
        self.clf = self.clf.fit(self.dataset['features']['training'], self.dataset['labels']['training'])
    ...
    def predict(self):
        ...
        :return:
        """
        self.predictions = self.clf.predict(self.dataset['features']['test'])
        LOGGER.info('Generated predictions')

    def evaluate(self):
        self.trnaccuracy = self.clf.score(self.dataset['features']['training'],
                                          self.dataset['labels']['training'],
                                          sample_weight=self.dataset['weights']['training'])
        self.tstaccuracy = self.clf.score(self.dataset['features']['test'],
                                          self.dataset['labels']['test'],
                                          sample_weight=self.dataset['weights']['test'])
...
def estimate_best_n_neighbours():
    ...
    """
    n_neighbours_range = np.arange(0.0, 1.0, 0.2)
